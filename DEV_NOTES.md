...
# PR Goal（feat/prompt-eval-metrics）

## 🎯 目标

建立一套最小可执行的提示词评估基线，用于回答：

- Router 提示词（`_classify_topic_profile`）是否把话题正确分到 `general/job/finance`？
- 路由后检索（domain profile）是否提升有效信息命中与生成质量？

## 范围（本 PR）

- 定义评估样本格式（topic + gold_profile + 可选备注）
- 提供 Router 评估脚本（准确率/召回率/混淆矩阵）
- 输出评估报告模板（基线结果 + 对比结果）
- 在文档中固化“如何跑评估、如何解读指标”
